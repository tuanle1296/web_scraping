...
def main():
    parser = argparse.ArgumentParser(description='TruyenFullVision Crawler (Docker Optimized)')
    ...
    with Base(True) as crawl:
        ...
        if crawl.wait_for_page_load(15):
            pagination_elem = crawl.find_element(lo.pagination)
            if pagination_elem is None:
                urls.append(main_url)
            else:
                total_pages_elems = crawl.find_elements(lo.page_link, pagination_elem)[:-1]
                page_range = len(total_pages_elems)
                for i in range(1, page_range + 1):
                    urls.append(f"{main_url}trang-{i}/#list-chapter")
        else:
