...
from src.locators import tam_giac_mua_he as lo
...
def crawl_worker(chapter_data, folder_name):
    ...
    for chap_url, chap_num in chapter_data:
        ...
        if (content == ""):
            ...
            crawl.capture_screen(title)
            crawl.define_img_name(title)
        # chapter_content = crawl.find_element(lo.chapter_content)
        # img_tags = crawl.find_elements(lo.img_tag, chapter_content)
        # if (img_tags):
        #     title = "chapter_" + str(chap_num)
        #     content = crawl.extract_images_to_file(img_tags, title)
        #     crawl.add_text_to_doc_file(title, content, title + ".docx")
        else:
...
def main(folder_name):
    ...
    main_url = "https://tuncoi.wordpress.com/liet-do-giai-tong/"
...
if __name__ == '__main__':
    main("tam_giac_mua_he")

'''Note: run this code using cmd: uv run test/tam_giac_mua_he.py'''