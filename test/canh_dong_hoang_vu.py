...
from src.locators import canh_dong_hoang_vu as lo
...
def crawl_worker(chapter_data, folder_name):
    ...
    for chap_url, chap_num in chapter_data:
        ...
        if not crawl.wait_for_page_load(10):
            ...
            continue
        isVisible = crawl.is_element_visible(lo.password_input_field)
        if (isVisible):
...
def main(folder_name):
    ...
    main_url = "https://lamsonthuymac.wordpress.com/2021/07/15/canh-dong-hoang-vu/"
    crawl.go_to_webpage(main_url)
    crawl.wait_for_page_load(10)
    urls_list = crawl.find_elements(lo.chap_list)
...
if __name__ == '__main__':
    main("canh_dong_hoang_vu")

'''Note: run this code using cmd: uv run test/canh_dong_hoang_vu.py'''
